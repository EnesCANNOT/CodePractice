...
fun main() {
    ...
    val largestProduct = findLargestProduct(grid)
    println("The greatest product of four adjacent numbers in the grid is: $largestProduct")
}

fun findLargestProduct(grid: Array<IntArray>): Long {
    ...
    for (row in 0 until rowCount) {
        for (col in 0 until colCount - 3) {
            val product: Long = (grid[row][col].toLong()
                    * grid[row][col + 1].toLong()
                    * grid[row][col + 2].toLong()
                    * grid[row][col + 3].toLong())
    ...
    for (row in 0 until rowCount - 3) {
        for (col in 0 until colCount) {
            val product: Long = (grid[row][col].toLong()
                    * grid[row + 1][col].toLong()
                    * grid[row + 2][col].toLong()
                    * grid[row + 3][col].toLong())
            if (product > maxProduct) {
                maxProduct = product
            }
        }
    ...
    for (row in 0 until rowCount - 3) {
        for (col in 0 until colCount - 3) {
            val product: Long = (grid[row][col].toLong()
                    * grid[row + 1][col + 1].toLong()
                    * grid[row + 2][col + 2].toLong()
                    * grid[row + 3][col + 3].toLong())
            if (product > maxProduct) {
                maxProduct = product
            }
        }
